...
def bookingForm():
    if 'userName' in session:
        return render_template('bookingForm.html',flights=flights,userName=session['userName'],rating = session['booking_system'].x)
    else:
        return redirect('/')
    ...
    if request.method == 'POST':
        ...
        if userName in users:
            if users[userName] == password:
                if 'userName' in session:
                    return render_template('bookingForm.html',flights=flights,userName=userName,rating= session['booking_system'].x)
                else:
...
def bookingConfirm():
    if request.method == 'POST':
        ...
        if flight not in bookedFlights:
            ...
            modalMessage += '</ul>'            
            response = make_response(jsonify({"modalMessage": modalMessage, "openSurvey": openSurvey }), 200)
            return response
        else:
            response = make_response(jsonify({"modalMessage": "Flight already booked"}), 200)
            return response
        
@app.route('/bookedFlights')
def viewBooked():
    print(bookedFlights)
    return render_template('bookedFlights.html',flights=bookedFlights,userName=session['userName'],rating= session['booking_system'].x)
...
def cancelBooking():
    if request.method == "POST":
        ...
        if booking_system.notCancelable:
            ...
            response = make_response(jsonify({"modalMessage": "Booking cannot be cancelled"}), 200)
            return response
